...
func extractAndValidateToken(authHeader, base64Key string) (string, error) {
	user, token, ok := parseBasicAuth(authHeader)
	if !ok {
		return "", errors.New("invalid basic auth format")
	}
	if err := VerifyPasetoToken(base64Key, token); err != nil {
		return "", errors.New("invalid PASETO token: " + err.Error())
	}
	return user, nil
}
...
Start:
	...
	authMethod := strings.ToLower(s.config.AuthMethod)
	if authMethod == "paseto" {
		...
		// Not using Paseto auth
		if len(s.config.Accounts) > 0 {
			// Use Basic Auth if accounts exist
		...
		// else: no auth at all if no accounts
	}
